...
namespace HealthBar.Rendering {
	public sealed class HealthBarRenderer : IRenderer, IDisposable {
		...
		public HealthBarRenderer(ICoreClientAPI api, ModConfig config) {
			_capi = api;
			...
			ColorUtil.ToRGBAVec4f(ColorUtil.Hex2Int(config.FrameColor ?? "#CCCCCC"), ref _frameCol);
		...
		public void OnRenderFrame(float dt, EnumRenderStage stage) {
			...
			_opacity = Clamp01(_opacity + dt / (IsVisible ? config.FadeInSpeed : -config.FadeOutSpeed));
			if (_opacity <= 0)
			...
			var w = scale * config.BarWidth;
			var h = scale * config.BarHeight;
			var x = (float)scr.X - w / 2f;
			var y = _capi.Render.FrameHeight - (float)scr.Y - h - config.VerticalOffset;
		...
		private void UpdateHealthColor(float pct) {
			var hex = pct <= config.LowHealthThreshold ? config.LowHealthColor
				: pct <= config.MidHealthThreshold ? config.MidHealthColor
				: config.FullHealthColor;
